...
//파일 업로드 관련
function mh_board_insert_attachment($file_handler,$post_id,$setthumb=false) {// 082
...
function mh_board_action(){
	...
	if(@$_REQUEST['mh_action'] == 'post' && wp_verify_nonce($_REQUEST['_mh_board_nonce'],'mh_board_nonce')){ //글쓰기 관련
		...
		if($pid = $mh_board_write->register_post()){
			...
			/* 082 START */$redirect_url = !empty($_POST['redirect_to']) ? $_POST['redirect_to'] : mh_get_board_link_by_board_cat($_POST['board_category']);
			wp_redirect($redirect_url);/* 082 END */
		}else{
		...
		if((get_current_user_id() == $post->post_author && get_current_user_id() > 0) || (isset($_POST['guest_password']) && $_POST['guest_password'] == $guest_info['guest_password'])){
			...
			if(wp_update_post($args)){
				/* 082 START */$redirect_url = !empty($_POST['redirect_to']) ? $_POST['redirect_to'] : get_post_type_archive_link('board');
				wp_redirect($redirect_url);/* 082 END */
