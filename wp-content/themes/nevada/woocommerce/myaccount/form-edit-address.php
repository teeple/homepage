...
<?php else : ?>
	...
	<form action="<?php echo esc_url( add_query_arg( 'address', $load_address, get_permalink( woocommerce_get_page_id('edit_address') ) ) ); ?>" method="post">
		...
		<h3 class="home-title"><span><?php if ($load_address=='billing') _e('Billing Address', 'woocommerce'); else _e('Shipping Address', 'woocommerce'); ?></span></h3>
		...
		<input type="submit" class="woo-button" style="float:right; margin-bottom:20px;" name="save_address" value="<?php _e('Save Address', 'woocommerce'); ?>" />
